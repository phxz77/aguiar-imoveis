    ...
    <div className="space-y-4">
      ...
      >
        <input
          ...
          className="hidden"
          onChange={(e) => {
            console.log("[UPLOAD-DEBUG] <input type=file> onChange disparado. Arquivos selecionados:", e.target.files?.length ?? 0);
            if (e.target.files && e.target.files.length > 0) {
              processFiles(e.target.files);
              e.target.value = ""; // reseta input para permitir selecionar os mesmos arquivos novamente
            } else {
              console.warn("[UPLOAD-DEBUG] onChange disparou mas e.target.files está vazio/nulo.");
            }
          }}
        />
